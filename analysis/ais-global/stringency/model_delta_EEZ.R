...
# 
# # Extract data for each combination  
# data_dens <- bind_rows(foreach(i = 1:nrow(combinations), .packages=c("dplyr", "raster", "exactextractr")) %dopar% { 
#   
#   # select file
#   ivar <- combinations$var[i]
#   idate_pre <- combinations$date_pre[i]
#   tif_file <- list.files(input_dir, full.names = TRUE, recursive=TRUE, pattern=sprintf("%s_%s_dens_moll.tif$", format(idate_pre, "%Y%m%d"), ivar))
# 
#   # import raster
#   rast <- raster(tif_file)
#   
...
#   
#   # generate data.frame
#   df <- data.frame(id = poly_moll$MRGID, var = ivar, date = idate_pre, measure = "delta",
#                    mean = extr_mean)
#   df
# })
# 
...
# 
# #--------------------------------------------------
# # 4. Combine delta and densities
# #--------------------------------------------------
# 
# # prepapre density
# data_dens <- data_dens %>%
#   rename(dens2019 = mean) %>%
#   mutate(month = month(date)) %>%
#   dplyr::select(id, var, month, dens2019)
# 
# # filter data
# data <- data %>%
#   mutate(month = month(date)) %>%
#   left_join(data_dens, by = c("id" = "id", "var" = "var", "month" = "month"))
...
month_sum <- data %>%
  group_by(date, var) %>%
  summarize(n = n(),
            delta_avg = mean(mean, na.rm=TRUE))
...
# all together, median
p1 <- ggplot(filter(data, var == "COUNT"), aes(x = date)) +
  geom_line(aes(y = median, group = TERRITORY1), size = 0.7, color="grey50", alpha=0.2) +
  geom_line(data=filter(month_sum, var == "COUNT"), aes(x=date, y = delta_avg), size = 1, color="black", alpha=0.8) +
  #geom_ribbon(data=month_sum, aes(x=date, ymin = delta_avg-delta_sd, ymax = delta_avg+delta_sd), fill="#3182bd", alpha=.2, linetype=0) +
  geom_hline(yintercept = 0, linetype="dotted") +
  scale_x_date(date_breaks = "1 month", date_labels = "%b", expand = c(0, 0)) +
  ylab(expression(Absolute~change~(Delta~vessels~km^-2~month^-1))) +
...
out_file <- paste0(output_data, "/", jvar, "_eez_med_iqr.png")
ggsave(out_file, p, width=10, height=10, units = "cm")


#-------------------------------------------------------
# Select countries with major changes
#-------------------------------------------------------

m0 <- lm(median ~ TERRITORY1, data = data)
sort(m0$coefficients)

sel_countries <- c("Spain", "France", "Italy", "China", "India",
                   "United States", "South Korea", "Indonesia", "United Kingdom",
                   "Belgium", "Poland", "Lithuania", "Germany", "Iran", "Netherlands",
                   "Bahrain", "United Arab Emirates", "Qatar", "Montenegro", "Taiwan")
...
for(j in 1:length(vars)){
  ...
  # model
  m1 <- lmer(median ~ SiAvg*income + (1|ISO_SOV1), data = jdata)
...
for(j in 1:length(vars)){
  ...
  #
  out_file <- paste0(output_data, "/", jvar, "_int_delta.png")
  ggsave(out_file, p, width=10, height=10, units = "cm")
}
