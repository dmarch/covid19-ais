#------------------------------------------------------------------------------
# extract_count_by_EEZ
#------------------------------------------------------------------------------
# This script extract vessel count data per EEZ for each variable and timing
...
# set input directory
input_dir <- "data/out/ais-global/count/"
...
for (i in 1:length(dates)){  ## process monthly data
  for (j in 1:length(vars)){  ## process variable 
    ...
    tif <- list.files(input_dir_month, full.names = TRUE, pattern = sprintf("%s_%s_cnt.tif$", idate, jvar))
    cnt <- raster(tif)
...
data <- data %>% 
  rename(lon = x, lat = y) %>%
  dplyr::select(idcell, lon, lat, date, var, vessels, eez_km2, MRGID)
...
# export data.frame
write.csv(data, paste0(out_dir, "eez_ais_count.csv"), row.names = FALSE)
